...
const CurrentPlayerStage = ({ auctionState }) => {
    if (auctionState?.status === 'pending') {
        return (
            <div className="h-full flex items-center justify-center bg-black bg-opacity-20 rounded-xl border-2 border-dashed border-gray-700">
                <p className="text-2xl text-gray-500">Auction is about to begin...</p>
            </div>
        );
    }
    if (auctionState?.status === 'finished') {
        return (
            <div className="h-full flex items-center justify-center bg-black bg-opacity-20 rounded-xl border border-gray-700">
                <p className="text-4xl font-bold text-orange-500">AUCTION FINISHED!</p>
            </div>
        );
    }
    if (!auctionState?.currentPlayer) {
        return (
            <div className="h-full flex items-center justify-center bg-black bg-opacity-20 rounded-xl border border-gray-700">
                <p className="text-2xl text-gray-500">Waiting for next player...</p>
            </div>
        );
    }
    ...
    <div className="grid grid-cols-5 gap-3 h-full">
        {teams.map(team => {
            ...
            return (
                <div
                    key={team.id}
                    className={`bg-black bg-opacity-20 rounded-lg p-3 flex flex-col justify-between border ${borderColor} ${!canAfford ? 'opacity-30' : ''} transition-all relative`}
                >
            ...
            {history.map((item, i) => (
                <div key={i}>
                    ...
                    <p className="text-xs text-gray-400">
                        to {item.team} for ₹{Number(item.price).toLocaleString('en-IN')}
                    </p>
                </div>
            ))}
        </div>
...
const AuctioneerControls = ({ auctionState, onStart, onNext }) => (
    <div className="bg-black bg-opacity-20 rounded-xl p-4 border border-gray-700">
        ...
        {auctionState?.status === 'pending' && (
            <button onClick={onStart} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-lg">
                START AUCTION
            </button>
        )}
        {auctionState?.status === 'sold' && (
            <button onClick={onNext} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-lg">
                NEXT PLAYER
            </button>
        )}
    </div>
